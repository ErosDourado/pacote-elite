...
import { TrendingUp, Users, Calendar, CreditCard, Check, X, Clock, Package, Plus, Minus, CalendarDays } from 'lucide-react'
...
function OverviewTab() {
  const { appointments, finance, updateAppointmentStatus } = useApp()
  ...
  const periodRevenue = filteredTx.filter(a => a.paymentStatus === 'paid').reduce((s, a) => s + (a.service?.price ?? 0), 0)
          ...
          {filteredTx.map((a, i, arr) => {
            ...
            return (
              <div key={a.id} className="px-4 py-3.5"
                ...
                {/* Botões de ação — só exibidos se status ainda não finalizado */}
                {!isRefunded && (
